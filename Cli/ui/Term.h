...
// Состояния различают ВОЛНУ, а не машину-источник, и это не упрощение.
// Откуда пришёл конкретный чанк — из окна, от пира или от отправителя —
// знает только сервер: получателю фрейм приходит одинаковым, и это та самая
// причина, по которой веер не копирует полезную нагрузку. Зато волна
// видна по порядку прибытия — и в карте важна именно она: видно, как
// хвост едет вместе со всеми, а начало тянется отдельно.
//
// Чем за это плачено — сколько пришло из окна, от пиров и от отправителя —
// говорит отдельная строка по сообщению stats от сервера.
enum class Seg : uint8_t {
    None = 0,      // нет                     ·
    Inflight,      // в пути                 ░
    Have,          // было до запуска        ▒
    Backfill,      // приехало второй волной ▓
    Live,          // приехало живым потоком  █
};
