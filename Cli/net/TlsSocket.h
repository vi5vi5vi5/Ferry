...
public:
    ...
    //
    // Одно чтение отдаёт не больше ОДНОЙ записи TLS, то есть 16 КиБ, каким
    // бы большим ни был буфер. Это не наша особенность, а устройство
    // протокола, и на нём легко обжечься: цикл, который после короткого
    // чтения решает, что данные кончились, на TLS будет забирать по 16 КиБ
    // за оборот, сколько бы их ни пришло.
    int read(void *buf, size_t len);
    int write(const void *buf, size_t len);

    // Сколько байт уже расшифровано и лежит внутри OpenSSL.
    //
    // Нужно ровно для одного, зато насмерть: ядро об этих байтах ничего не
    // знает, select их не видит, и цикл событий, спросив только сокет,
    // уснул бы на полном таймауте с готовыми данными в руках.
