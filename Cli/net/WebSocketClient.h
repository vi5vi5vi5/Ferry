...
public:
    ...
    // Протолкнуть очередь в сокет сейчас, не дожидаясь pump и ничего не
    // читая. Отправитель зовёт это после каждого чанка: иначе сокет
    // простаивал бы, пока готовится следующий.
    bool flush() { return flushOutgoing(); }
