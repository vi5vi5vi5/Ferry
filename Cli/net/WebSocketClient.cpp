...
bool WebSocketClient::pump(int timeoutMs)
{
    ...
    // Расшифрованный хвост внутри OpenSSL ядру не виден, и select про него
    // промолчит. Спрашиваем сокет, но не ждём на нём.
    const bool buffered = m_socket.pending() > 0;

    bool readable = false, writable = false;
    if (!waitFor(m_socket.handle(), true, wantWrite, buffered ? 0 : timeoutMs, &readable,
                 &writable)) {
    ...
    if (readable) {
        // Вычерпываем до конца, а не одним чтением.
        //
        // Здесь была самая дорогая ошибка клиента: цикл выходил, как
        // только чтение вернуло меньше, чем размер буфера. На голом TCP
        // это почти работает, а на TLS не работает никогда — SSL_read
        // отдаёт не больше одной записи, то есть 16 КиБ при буфере в 64,
        // и условие выхода срабатывало ВСЕГДА, с первого же оборота.
        // Получался один заход в цикл событий на каждые 16 КиБ: между
        // ними — перерисовка панели и select. Приём упирался в сотни
        // килобайт в секунду на канале в десятки мегабайт, причём отдача
        // при этом летела на полной скорости и не давала заподозрить
        // клиента (отправитель за оборот цикла успевает отдать всё, что
        // влезает в сокет, и панель у него стоит копейки на мегабайт).
        //
        // Выход теперь ровно один: чтение вернуло 0, то есть данных
        ...
        for (;;) {
            ...
            // Потолок на оборот. Без него быстрый сервер держал бы нас
            // здесь неограниченно долго, набивая m_ready — а разгребает
            // её вызывающий, и до него надо вернуться. Восемь мегабайт —
            // это и достаточно, чтобы расходы на оборот цикла стали
            // незаметны, и достаточно мало, чтобы панель не застывала.
            drained += size_t(n);
            if (drained >= kDrainBudget)
                break;
        }
    }
